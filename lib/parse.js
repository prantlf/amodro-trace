...
define(['esprima', './lang'], function (esprima, lang) {
    ...
    function traverse(object, visitor, parent) {
        ...
        for (var i = 0, keys = Object.keys(object); i < keys.length; i++) {
            child = object[keys[i]];
            if (typeof child === 'object' && child !== null && child !== parent) {
    ...
    function parse(moduleName, fileName, fileContents, options) {
        ...
        parse.recurse(astRoot, null, function (callName, config, name, deps, node, factoryIdentifier, fnExpScope) {
            if (!deps) {
                deps = [];
            }
     ...
     * @param {Object} object the AST node to start with.
     * @param {Object} parent the previously visited AST node or null.
     ...
     */
    parse.recurse = function (object, parent, onMatch, options, fnExpScope) {
        ...
        if (hasHas && object.type === 'IfStatement' && object.test.type &&
            ...
            if (object.test.value) {
                //Take the if branch
                this.recurse(object.consequent, object, onMatch, options, fnExpScope);
            } else {
                //Take the else branch
                this.recurse(object.alternate, object, onMatch, options, fnExpScope);
            }
        } else {
            ...
            for (i = 0, keys = Object.keys(object); i < keys.length; i++) {
                child = object[keys[i]];
                if (typeof child === 'object' && child !== null && child !== parent) {
                    result = this.recurse(child, object, onMatch, options, fnExpScope);
    ...
    parse.renameNamespace = function (fileContents, ns) {
        ...
        parse.recurse(astRoot, null, function (callName, config, name, deps, node) {
            locs.push(node.loc);
    ...
    parse.findDependencies = function (fileName, fileContents, options) {
        ...
        parse.recurse(astRoot, null, function (callName, config, name, deps) {
            if (deps) {
    ...
    parse.getAllNamedDefines = function (fileContents, excludeMap) {
        var names = [];
        parse.recurse(esprima.parse(fileContents), null,
